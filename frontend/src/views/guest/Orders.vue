      ...
      <div class="order-list">
        <div v-for="order in orders" :key="order.id" class="order-card">
          ...
          <div class="order-actions" v-if="order.status === 'PENDING' || order.status === 'CONFIRMED'">
            <button class="btn-cancel-order" @click="cancelOrder(order.id)">取消订单</button>
          </div>
        </div>
      </div>
    </main>
...
const loadOrders = async () => {
  const res = await api.get('/orders/guest')
...
.btn-cancel-order { padding: 6px 16px; border: 1px solid #e74c3c; color: #e74c3c; background: #fff; border-radius: 6px; cursor: pointer; font-size: 13px; }
